(function(b){"function"===typeof define&&define.amd?define(["jquery"],b):b("undefined"!=typeof jQuery?jQuery:window.Zepto)})(function(b){function z(a){var h=a.data;a.isDefaultPrevented()||(a.preventDefault(),b(a.target).ajaxSubmit(h))}function v(a){var h=a.target,d=b(h);if(!d.is("[type=submit],[type=image]")){h=d.closest("[type=submit]");if(0===h.length)return;h=h[0]}var c=this;c.clk=h;"image"==h.type&&(void 0!==a.offsetX?(c.clk_x=a.offsetX,c.clk_y=a.offsetY):"function"==typeof b.fn.offset?(d=d.offset(),
...
function c(c){function d(a){var b=null;try{a.contentWindow&&(b=a.contentWindow.document)}catch(c){r("cannot get iframe.contentWindow document: "+c)}if(b)return b;try{b=a.contentDocument?a.contentDocument:a.document}catch(e){r("cannot get iframe.contentDocument: "+e),b=a.document}return b}function f(){function a(){try{var b=d(w).readyState;r("state = "+b);b&&"uninitialized"==b.toLowerCase()&&setTimeout(a,50)}catch(c){r("Server abort: ",c," (",c.name,")"),h(2),v&&clearTimeout(v),v=void 0}}var c=n.attr2("target"),
k=n.attr2("action"),m=n.attr("enctype")||n.attr("encoding")||"multipart/form-data";l.setAttribute("target",t);g&&!/post/i.test(g)||l.setAttribute("method","POST");k!=e.url&&l.setAttribute("action",e.url);e.skipEncodingOverride||g&&!/post/i.test(g)||n.attr({encoding:"multipart/form-data",enctype:"multipart/form-data"});e.timeout&&(v=setTimeout(function(){z=!0;h(1)},e.timeout));var p=[];try{if(e.extraData)for(var q in e.extraData)e.extraData.hasOwnProperty(q)&&(b.isPlainObject(e.extraData[q])&&e.extraData[q].hasOwnProperty("name")&&
e.extraData[q].hasOwnProperty("value")?p.push(b('<input type="hidden" name="'+e.extraData[q].name+'">').val(e.extraData[q].value).appendTo(l)[0]):p.push(b('<input type="hidden" name="'+q+'">').val(e.extraData[q]).appendTo(l)[0]));e.iframeTarget||x.appendTo("body");w.attachEvent?w.attachEvent("onload",h):w.addEventListener("load",h,!1);setTimeout(a,15);try{l.submit()}catch(u){document.createElement("form").submit.apply(l)}}finally{l.setAttribute("action",k),l.setAttribute("enctype",m),c?l.setAttribute("target",
c):n.removeAttr("target"),b(p).remove()}}function h(a){if(!k.aborted&&!C)if(q=d(w),q||(r("cannot access response document"),a=2),1===a&&k)k.abort("timeout"),y.reject(k,"timeout");else if(2==a&&k)k.abort("server abort"),y.reject(k,"error","server abort");else if(q&&q.location.href!=e.iframeSrc||z){w.detachEvent?w.detachEvent("onload",h):w.removeEventListener("load",h,!1);a="success";var c;try{if(z)throw"timeout";var f="xml"==e.dataType||q.XMLDocument||b.isXMLDoc(q);r("isXml="+f);if(!f&&window.opera&&
(null===q.body||!q.body.innerHTML)&&--B){r("requeing onLoad callback, DOM not available");setTimeout(h,250);return}var g=q.body?q.body:q.documentElement;k.responseText=g?g.innerHTML:null;k.responseXML=q.XMLDocument?q.XMLDocument:q;f&&(e.dataType="xml");k.getResponseHeader=function(a){return{"content-type":e.dataType}[a.toLowerCase()]};g&&(k.status=Number(g.getAttribute("status"))||k.status,k.statusText=g.getAttribute("statusText")||k.statusText);var l=(e.dataType||"").toLowerCase(),m=/(json|script|text)/.test(l);
if(m||e.textarea){var n=q.getElementsByTagName("textarea")[0];if(n)k.responseText=n.value,k.status=Number(n.getAttribute("status"))||k.status,k.statusText=n.getAttribute("statusText")||k.statusText;else if(m){var p=q.getElementsByTagName("pre")[0],t=q.getElementsByTagName("body")[0];p?k.responseText=p.textContent?p.textContent:p.innerText:t&&(k.responseText=t.textContent?t.textContent:t.innerText)}}else"xml"==l&&!k.responseXML&&k.responseText&&(k.responseXML=D(k.responseText));try{A=H(k,l,e)}catch(G){a=
"parsererror",k.error=c=G||a}}catch(F){r("error caught: ",F),a="error",k.error=c=F||a}k.aborted&&(r("upload aborted"),a=null);k.status&&(a=200<=k.status&&300>k.status||304===k.status?"success":"error");"success"===a?(e.success&&e.success.call(e.context,A,"success",k),y.resolve(k.responseText,"success",k),u&&b.event.trigger("ajaxSuccess",[k,e])):a&&(void 0===c&&(c=k.statusText),e.error&&e.error.call(e.context,k,a,c),y.reject(k,"error",c),u&&b.event.trigger("ajaxError",[k,e,c]));u&&b.event.trigger("ajaxComplete",
[k,e]);u&&!--b.active&&b.event.trigger("ajaxStop");e.complete&&e.complete.call(e.context,k,a);C=!0;e.timeout&&clearTimeout(v);setTimeout(function(){e.iframeTarget?x.attr("src",e.iframeSrc):x.remove();k.responseXML=null},100)}}var l=n[0],m,e,u,t,x,w,k,z,v,y=b.Deferred();y.abort=function(a){k.abort(a)};if(c)for(m=0;m<p.length;m++)c=b(p[m]),E?c.prop("disabled",!1):c.removeAttr("disabled");e=b.extend(!0,{},b.ajaxSettings,a);e.context=e.context||e;t="jqFormIO"+(new Date).getTime();e.iframeTarget?(x=b(e.iframeTarget),
...
a+")")},H=function(a,c,e){var d=a.getResponseHeader("content-type")||"",f="xml"===c||!c&&0<=d.indexOf("xml");a=f?a.responseXML:a.responseText;f&&"parsererror"===a.documentElement.nodeName&&b.error&&b.error("parsererror");e&&e.dataFilter&&(a=e.dataFilter(a,c));"string"===typeof a&&("json"===c||!c&&0<=d.indexOf("json")?a=I(a):("script"===c||!c&&0<=d.indexOf("javascript"))&&b.globalEval(a));return a};return y}if(!this.length)return r("ajaxSubmit: skipping submit process - no element selected"),this;
...
this;f=b.param(u,m);l&&(f=f?f+"&"+l:l);"GET"==a.type.toUpperCase()?(a.url+=(0<=a.url.indexOf("?")?"&":"?")+f,a.data=null):a.data=f;var t=[];a.resetForm&&t.push(function(){n.resetForm()});a.clearForm&&t.push(function(){n.clearForm(a.includeHidden)});if(!a.dataType&&a.target){var z=a.success||function(){};t.push(function(c){var d=a.replaceTarget?"replaceWith":"html";b(a.target)[d](c).each(z,arguments)})}else a.success&&t.push(a.success);a.success=function(b,c,d){for(var f=a.context||this,g=0,h=t.length;g<
